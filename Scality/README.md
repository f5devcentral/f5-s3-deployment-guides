...
- **Service Port:** 80 (the HTTP port for the RING S3 service; specify 443 for an HTTPS endpoint)

Click **Add (4)** to save it to the member list. Then enter the next node's details and click **Add (4)** again until all nodes are added. In this example, three nodes provide the RING S3 service, so add the nodes at 10.150.91.141 through 10.150.91.143 before completing the pool. Once every node appears in the member list, click **Finished (5)**.
...
The S3 Services page provides access to the S3 management applications. Use the `S3 Console` to create an account, an S3 user, and access credentials. Click the **Log in (1)** button.
![S3 Services](./assets/scality_s3services.png)

To create a test account, click the **Create account (1)** button on the `Accounts` page.
...
Click the **Submit (5)** button to create the account.

The newly created account will be added to the accounts list:
...
In the login form, enter the newly created account's credentials:
...
To connect to the RING S3 service, the user needs an access key ID and a secret access key. Click the **+ Generate a new key (1)** button.
![Create S3 user result](./assets/scality_warp_s3_user_access_keys.png)

A new access key pair will be generated. Save the **AccessKey (1)**, then click the **Show (2)** button next to **SecretAccessKey** and save that value as well. You will need both later. Do not share these credentials with anyone.
...
In the Scality RING management interface, select **S3 Services (1)** and click **Login (2)** to open the `S3 Browser`.
![Scality RING S3 Browser login](./assets/scality_ring_s3browser.png)

Then use the saved access key ID and secret access key to log in. Enter them in the **Access Key (1)** and **Secret key (2)** fields.
...
#### 2.7.2 Use Warp to Test Connectivity to the RING S3 Service Through the BIG-IP Virtual Server

Warp is an S3 benchmarking tool. It drives a representative S3 workload (mixed PUT, GET, DELETE, and STAT operations) against an endpoint and reports throughput, latency, and error counts. Pointing Warp at the BIG-IP virtual server address, rather than directly at a node providing the RING S3 service, confirms that traffic flows end to end through the load balancer and gives a baseline measurement of RING's performance behind the BIG-IP.
...
Configure Warp with the BIG-IP virtual server address as the S3 endpoint, along with the access key ID and secret access key generated in RING, and run a short mixed-workload test:
...
An error count of zero confirms that every request reached the RING S3 service and returned a successful response through the virtual server.
