...
- [2. BIG-IP Configuration](#2-big-ip-configuration)
  ...
  - [2.7 Test the Virtual Server](#27-test-the-virtual-server)
    ...
    - [2.7.2 Use Warp to Test Connectivity to the StorageGRID S3 Cluster Through the BIG-IP Virtual Server](#272-use-warp-to-test-connectivity-to-the-storagegrid-s3-cluster-through-the-big-ip-virtual-server)
...
![Lab setup overview showing BIG-IP, NetApp StorageGRID cluster, and S3 clients](./assets/setup_overview.png)
...
| --------- | ---------------- | ---------------- | -------------- |
| BIG-IP    | vlan1.1-internal | 10.150.91.130/24 | 1.1 (untagged) |
| BIG-IP    | vlan1.2-external | 10.150.92.130/24 | 1.2 (untagged) |
...
To verify that the StorageGRID S3 endpoint is reachable, create a custom monitor in BIG-IP. Go to **Local Traffic (1) > Monitors (2)** and click **Create (3)**.
![BIG-IP custom monitor list with the Create button highlighted](./assets/bigip_monitor_create.png)

- **Type (1):** `HTTPS` or `HTTP`, depending on how your StorageGRID S3 endpoint is configured.
- **Name (2):** `Options_HTTPS_StorageGRID`
...
![Lab setup overview showing BIG-IP, StorageGRID cluster, and S3 clients](./assets/setup_overview.png)
...
- **Name (1):** `storageGRID_Pool`
- **Health Monitor (2):** `Options_HTTPS_StorageGRID`
...
![Pools list showing the new storagegrid-cluster pool](./assets/bigip_pool_result.png)
...
- **Name (1):** `StorageGRID_S3_Virtual_Server`
...
- **6. SSL Profile (Server) (6):** Set to the default server SSL profile (`serverssl`) if your StorageGRID cluster serves encrypted S3 traffic.
...
In the **Resources** section, select the `storageGRID_Pool` pool from the **Default Pool (1)** dropdown to bind the virtual server to the backend nodes.

Click **Finished (2)** to save the virtual server.
![Virtual Server Resources section with storagegrid-cluster selected as the Default Pool](./assets/bigip_vs_create_details_3.png)
...
![Virtual Servers list showing storagegrid-cluster-vs with a green available status](./assets/bigip_vs_result.png)
...
![StorageGRID Nodes](./assets/storagegrid_tenant_create.png)
...
![StorageGRID Tenant Create Bucket ](./assets/storagegrid_tenant_acme_create_bucket.png)

Enter `warp-connectivity-test` as the **Bucket name (1)** and click **Continue (2)**:
![StorageGRID Tenant Create Bucket - Name ](./assets/storagegrid_tenant_acme_create_bucket_details_1.png)

Scroll down **(1)** through the features list until you see the **Create bucket (2)** button, then click it.
![StorageGRID Tenant Create Bucket - Name ](./assets/storagegrid_tenant_acme_create_bucket_details_2.png)

After the bucket is created, click **Finish (1)**:
![StorageGRID Tenant Create Bucket - Name ](./assets/storagegrid_tenant_acme_create_bucket_details_3.png)
...
![StorageGRID Tenant Create User ](./assets/storagegrid_tenant_acme_create_user.png)
...
![StorageGRID Tenant Create User - Properties](./assets/storagegrid_tenant_acme_create_user_details_1.png)

Click **Create user (1)** in the last wizard step:
![StorageGRID Tenant Create User - Create User](./assets/storagegrid_tenant_acme_create_user_details_2.png)
...
![StorageGRID Tenant Create Access Key](./assets/storagegrid_tenant_acme_access_key_user.png)

Select the **Access keys (1)** tab and click **Create key (2)**:
![StorageGRID Tenant Create Access Key](./assets/storagegrid_tenant_acme_access_key_details_1.png)
...
![StorageGRID Tenant Create Access Key](./assets/storagegrid_tenant_acme_access_key_details_2.png)

Copy and save the **Access key ID (1)** and **Secret access key (2)**. Click **Finish (3)**.
![StorageGRID Tenant Create Access Key](./assets/storagegrid_tenant_acme_access_key_details_3.png)

The created access key will appear in the user's access key list:
![StorageGRID Tenant Create Access Key](./assets/storagegrid_tenant_acme_access_key_result.png)

#### 2.7.2 Use Warp to Test Connectivity to the StorageGRID S3 Cluster Through the BIG-IP Virtual Server

Warp is an S3 benchmarking tool. It drives a representative S3 workload (mixed PUT, GET, DELETE, and STAT operations) against an endpoint and reports throughput, latency, and error counts. Pointing Warp at the BIG-IP virtual server address, rather than at an individual StorageGRID node, confirms that traffic flows end to end through the load balancer and gives a baseline measurement of the cluster's performance behind the BIG-IP.
...
Configure Warp with the BIG-IP virtual server address as the S3 endpoint, along with the access key ID and secret access key generated on the StorageGRID cluster, and run a short mixed-workload test:
